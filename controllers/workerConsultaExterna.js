...
const calculoMonoxidoCarbono = require('./calculo_indices/calculoMonoxidoCarbono.js');
...
parentPort.on('message', async (message) => {
    if (message.command === 'startAnalysis') {
        try {
            ...
            // privateKey no es necesario, la inicialización lo maneja
            const fecha_filtro = workerData.fecha_filtro;
            const linea_productiva = workerData.linea_productiva;

            // ensure Earth Engine initialized
            await initializeEarthEngine();
            // ahora continuar con el análisis
                    ...
                    const filteredFeatureCollection = ee.FeatureCollection(geojson).set('crs', 'EPSG:4326');
                    ...
                    var fechaFinObj = new Date(fecha_fin);
                    ...
                    var imageList = sortedCollection.toList(20);
                    //console.log(sortedCollection.getInfo(),'sortedCollection');
                    
                    console.log("AAAAAAAAA----------------MIKI");
                    //console.log(sortedCollection.getInfo());
                    ...
                    if (count === 0) {
                        ...
                        var dates = sortedCollection.limit(20).aggregate_array('system:time_start').getInfo();
                        dates = dates.map(date => ee.Date(date).format('YYYY-MM-dd').getInfo());
                        console.log(dates);
                        console.log('N° Imágenes en este rango de fechas: '+count);
                    ...
                    */
                    ...
                    dates.forEach((date, index) => {
                      ...
                      if (!seenMonths[key]) {
                        seenMonths[key] = true;
                        firstDayIndices.push(index);
                      }
                    });

                    console.log(firstDayIndices);
                    ...
                    for (var i = 0; i < firstDayIndices.length; i++) {

                      if(i==3) break;

                      var image = ee.Image(imageList.get(firstDayIndices[i]));
                      ...
                      if (image) {
                          ...
                          var Sentinel2A = ee.ImageCollection([image]).map(maskS2clouds);
                          var Sentinel2Amedian = Sentinel2A.mosaic().clip(roi)
                                              .select(['B2','B3','B4','B8','B11','B12'])
                                              .rename(['BLUE', 'GREEN', 'RED', 'NIR', 'SWIR1', 'SWIR2']);
                          var dict_indices = {
                              url: url,
                              ndvi: calculoNdvi(Sentinel2Amedian, roi, roi0),
                              // Otros índices si es necesario
                              //precipitacion: calculoPrecipitacion(fecha, roi),
                          };
                          dict_indices = await calcularAreaGeoJSON(dict_indices);
                          resultsByDate["indices"][date] = dict_indices;

                      } else {
                          console.log(`No image available for date: ${date}`);
                      }
                  }

                    console.log(fecha, roi);

                    resultsByDate["precipitacion"] = calculoPrecipitacion(fecha, roi);
                    resultsByDate["temperatura"] = calculoTemperatura(fecha, roi);
                    resultsByDate["monoxidoCarbono"] = calculoMonoxidoCarbono(fecha, roi);

                    guardarGeoJSON(resultsByDate, geojson.features[0].properties.idg, 'unico', fecha_fin, linea_productiva);
                    ...
                    parentPort.postMessage({ command: 'analysisResult', result: resultsByDate });
...
async function guardarGeoJSON(geojson, gid, indice, fecha_indice, linea_productiva) {

  try {
    const query = `
      INSERT INTO externo_historial_indice_parcela_cafe (gid, indice, fecha_indice, geojson, linea_productiva)
      VALUES ($1, '$2', '$3', '$4', '$5')
    `;
