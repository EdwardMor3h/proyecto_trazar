...
map.on('load', async function(){
    ...
    //calcular_intersecciones_parcelas();
    obtener_data();
...
/*
async function calcular_intersecciones_parcelas(){

  await $.ajax({
    url: '/actualizar-intersecciones-parcelas',
    method: 'GET',
    success: function(response){
      console.log(response, 'Ok');
    },
    error: function(error) {
      console.log('Error Interseccion');
    }
  });
}
*/

async function obtener_data(){
  ...
  $.ajax({
    ...
    data: {"fecha_creacion" : '2020-09-20'},
    success: function(geojson) {
      ...
      var estiloParcelas = {
        "fill-color": [
          'case',
          ['==', ['get', 'ints_anp'], '1'], '#A52A2A', // Color para ints_anp igual a '1'
          ['==', ['get', 'ints_za'], '1'], '#800080', // Color para ints_za igual a '1'
          ['==', ['get', 'ints_deforestacion_2014'], '1'], '#FFFF00', // Color para ints_deforestacion_2014 igual a '1'
          ['==', ['get', 'ints_deforestacion_2020'], '1'], '#FFC0CB', // Color para ints_deforestacion_2020 igual a '1'
          '#29A847' // Color predeterminado si ninguna condición se cumple
        ],
        "fill-opacity": 0.5
      }

      agregar_geojson('cafe','fill',geojson, _paint=null, {'text_field':'nombre', 'text_size': 20});
      //agregar_geojson('cafe','fill',geojson, _paint=estiloParcelas, {'text_field':'nombre', 'text_size': 20});
      agregar_geojson_borde('cafe_borde','line',geojson);


      geojson.features = geojson.features.slice(0, geojson.features.length - 3683);

      try{
        llenarSelectUsuarios(geojson);  
      }catch(e){
        console.log(e);
      }

      _llenarTabla(geojson);

    }
  });

  function llenarSelectUsuarios(geojson){

    var id_usuarios_array = [];
    
    for(var i=0; i < geojson.features.length; i++){
      let id_usuario = geojson.features[i].properties.auth_user_id;
      if (!(id_usuarios_array.includes(id_usuario))) {
        id_usuarios_array.push(id_usuario);
      }
    }

    console.log(id_usuarios_array);

    for (let i = 0; i < id_usuarios_array.length; i++) {
      let usuario = buscar_usuario(id_usuarios_array[i]);
      let item = {
        id : usuario.id,
  ...
  function _llenarTabla(geojson){

    // if(tableParcelas){
    //   $('#table-actualizar-datos tbody').empty();
    ...
    for(var i=0; i < geojson.features.length; i++){
      var propiedadesGeojson = geojson.features[i].properties;
      ...
      let bool_ints_anp;
      let area_ints_anp_m2 = geojson.features[i].properties.area_ints_anp_m2;
      if(geojson.features[i].properties.ints_anp != null){
        geojson.features[i].properties.ints_anp == '1' ? bool_ints_anp = '<span class="badge badge-light-danger">Sí</span>' : bool_ints_anp = '<span class="badge badge-light-success">No</span>';

        if (geojson.features[i].properties.ints_anp == '1') 
        {
      ...
      let bool_ints_za;
      let area_ints_za_m2 = geojson.features[i].properties.area_ints_za_m2;
      if(geojson.features[i].properties.ints_za != null){
        geojson.features[i].properties.ints_za == '1' ? bool_ints_za = '<span class="badge badge-light-danger">Sí</span>' : bool_ints_za = '<span class="badge badge-light-success">No</span>';

        if (geojson.features[i].properties.ints_za == '1') 
        {
      ...
      let area_ints_deforestacion_2014_m2 = geojson.features[i].properties.area_ints_deforestacion_2014_m2;
      if(geojson.features[i].properties.ints_deforestacion_2014 != null){
        geojson.features[i].properties.ints_deforestacion_2014 == '1' ? bool_ints_deforestacion_2014 = '<span class="badge badge-light-danger">Sí</span>' : bool_ints_deforestacion_2014 = '<span class="badge badge-light-success">No</span>';

        if (geojson.features[i].properties.ints_deforestacion_2014 == '1') 
        {
      ...
      let area_ints_deforestacion_2020_m2 = geojson.features[i].properties.area_ints_deforestacion_2020_m2;
      if(geojson.features[i].properties.ints_deforestacion_2020 != null){
        geojson.features[i].properties.ints_deforestacion_2020 == '1' ? bool_ints_deforestacion_2020 = '<span class="badge badge-light-danger">Sí</span>' : bool_ints_deforestacion_2020 = '<span class="badge badge-light-success">No</span>';

        if (geojson.features[i].properties.ints_deforestacion_2020 == '1') 
        {
      ...
      let area_ints_parcelas_perhusa_m2 = geojson.features[i].properties.area_ints_parcelas_perhusa_m2;
      if(geojson.features[i].properties.ints_parcelas_perhusa != null){
        geojson.features[i].properties.ints_parcelas_perhusa == '1' ? bool_ints_parcelas_perhusa = '<span class="badge badge-light-danger">Sí</span>' : bool_ints_parcelas_perhusa = '<span class="badge badge-light-success">No</span>';

        if (geojson.features[i].properties.ints_parcelas_perhusa == '1') 
        {
      ...
      var tr = '<tr>\
                  ...
                  '</td>\
                  <td>\
                      ' + propiedadesGeojson.nombre + '\
                  </td>\
                  <td>\
                      ' + propiedadesGeojson.productor_nombre + '\
                  </td>\
                  <td>\
                      ' + propiedadesGeojson.productor_dni + '\
                  </td>\
                  <td>\
                      ' + propiedadesGeojson.zona_descripcion + '\
                  </td>\
                  <td>\
                      ' + username + '\
                  </td>\
                  ...
                  <td class="options">\
                    <div class="actions-container">\
                      ...
                      <a class="btn btn-complementary btn-aprobar-parcela '+ ocultarAprobar +'" data-id="'+ propiedadesGeojson.id +'">\
                      <i class="fa fa-check me-2"></i>\
                      Aprobar\
                      </a>\
                      <a class="btn btn-complementary btn-anular-parcela '+ ocultarAnular +'" data-id="'+ propiedadesGeojson.id +'">\
                      <i class="fa fa-x me-2"></i>\
                      Anular\
                      </a>\
                      <a class="btn btn-sm btn-eliminar-parcela" data-id="'+ propiedadesGeojson.id +'">\
...
function __borrar_agregar_geojson(_id, _type, _geojson, _paint=null, _label=null){
  ...
  */
  ...
  map.on('click', 'cafe', async function (e) {
    ...
    var _id = e.features[0].properties.id;
...
function buscarEnTabla(id){
  console.log(id);
  // Buscar la fila con el id correspondiente
  var fila = $("#table-actualizar-datos tbody").find("td:first-child").filter(function () {
  ...
  var posicion = fila.offset().top - ($("#table-actualizar-datos").height() / 2);

  // Resaltar temporalmente la fila
  fila.addClass("resaltado");

  // Desplazar la tabla para que la fila resaltada esté en el centro
  $("#table-actualizar-datos").animate({scrollTop: posicion}, 1000);

  // Esperar 3 segundos y quitar el resaltado
  setTimeout(function () {
      fila.removeClass("resaltado");
  }, 3000);
}

function __borrar_agregar_geojson_borde(_id, _type, _geojson){
...
$('.table-actualizar-datos').on('click', '.btn-aprobar-parcela', function(){
  ...
  $.ajax({
      ...
      success: function(response) {
        ...
        Swal.fire({
          title: 'Cambios realizados',
          text: 'Se actualizó el estado de la unidad productiva en la base de datos',
          icon: 'success',
          showConfirmButton: false,
          timer: 1500
        })
        ...
        btn.hide();
        btn.parent().find('.btn-anular-parcela').removeClass('d-none');
        btn.parent().find('.btn-anular-parcela').css("display", "inline-block");
...
$('.table-actualizar-datos').on('click', '.btn-anular-parcela', function(){
  ...
  $.ajax({
      ...
      success: function(response) {
        ...
        btn.hide();
        btn.parent().find('.btn-aprobar-parcela').css("display", 'inline-block');
...
$('.table-actualizar-datos').on('click', '.btn-eliminar-parcela', function(){

  var btn = $(this);

  console.log("eliminar parcela");

  var id = $(this).attr('data-id');


  Swal.fire({
    title: "¿Está seguro de eliminar la parcela?",
    text: "Este cambio no será revertible",
    icon: "warning",
    ...
    customClass: {
      confirmButton: 'mi-clase-confirm', // Clase para el botón de confirmar
      cancelButton: 'mi-clase-cancel'   // Clase para el botón de cancelar
    }
  }).then((result) => {
    if (result.isConfirmed) {
      ...
      $.ajax({
          ...
          success: function(response) {
            ...
            Swal.fire({
              title: 'Cambios realizados',
              text: 'Se elimino la unidad productiva',
              icon: 'success',
              showConfirmButton: false,
              timer: 1500
            })
            ...
            btn.hide();
            btn.parent().find('.btn-aprobar-parcela').removeClass('d-none');
...
$('.btn-descargar').click(function(){
  console.log(1);
  exportar_excel_status();
})

function exportar_excel_status(){
    var table = $('.table')[0];
    ...
    // Excluir la última columna (por ejemplo, la última celda de cada fila)
    var range = XLSX.utils.decode_range(worksheet['!ref']);
    range.e.c = Math.max(range.s.c, range.e.c - 1); // Restar 1 al índice de la última columna
...
$('.btn-aprobar-usuario').click(function(){
  let filtro_usuarios_array = $('#select-filtro-aprobar').select2('data');
  console.log(filtro_usuarios_array);
  let query_filtro_usuarios_dict = [];
  let usuarios_nombres = '';

  if (filtro_usuarios_array.length > 0)
  {
    let valores_usuarios_string = '';
    for (let i = 0; i < filtro_usuarios_array.length; i++)
    {
      valores_usuarios_string += filtro_usuarios_array[i].id.toString();
      i < (filtro_usuarios_array.length - 1) ? valores_usuarios_string += ',' : null; 

      //llenar nombre de usuarios
      usuarios_nombres += filtro_usuarios_array[i].text;
      i < (filtro_usuarios_array.length - 1) ? usuarios_nombres += ', ' : null; 
    }

    //Añadir corchetes al inicio y fin de la cadena
    valores_usuarios_string = '[' + valores_usuarios_string + ']';

    // Agregar campo y valor al dict filtros - Cuenca Hidrografica
    query_filtro_usuarios_dict["where"] != null ? query_filtro_usuarios_dict["where"]["auth_user_id"] = valores_usuarios_string : query_filtro_usuarios_dict["where"] = {"auth_user_id" : valores_usuarios_string};
    // query_filtro_usuarios_dict["where"]["ints_anp"] = ["0"];
    // query_filtro_usuarios_dict["where"]["ints_za"] = ["0"];
    // query_filtro_usuarios_dict["where"]["ints_deforestacion_2014"] = ["0"];
    // query_filtro_usuarios_dict["where"]["ints_deforestacion_2020"] = ["0"];
    // query_filtro_usuarios_dict["where"]["ints_parcelas_perhusa"] = ["0"];
    console.log(query_filtro_usuarios_dict);

    // Convertir diccionario en cadena de texto
    query_filtro_usuarios_dict = JSON.stringify(query_filtro_usuarios_dict["where"])
    console.log(query_filtro_usuarios_dict);
    if (valores_usuarios_string != null) {
      aprobacion_masiva_parcelas(query_filtro_usuarios_dict, usuarios_nombres);
    }
  ...
  else
  {
    Swal.fire({
      ...
      text: 'Primero debe selecconar uno o más usuarios para aprobar parcelas de forma masiva'
    });
  }

});

// Aprobar masivamente parcelas según usuarios
function aprobacion_masiva_parcelas($dict_filtros, $user)
{

  $.ajax({
    url: '/update_aprobar_masiva_parcelas',
    data: {
    ...
    success: function (response) {
      ...
      Swal.fire({
        ...
        text: 'Se aprobaron todas las parcelas sin conflicto de: ' + $user,
        icon: 'success',
        showConfirmButton: false,
        timer: 2500
      });
    ...
    error: function (error) {
      console.error('Error al aprobar las parcelas: ', error);
    },
  });

}

// Implementación DataTable
var tableParcelas = null;

function cargar_data_parcelas_status(){

  tableParcelas = $('#table-actualizar-datos').DataTable({
    "paging":false,
    "language": {
